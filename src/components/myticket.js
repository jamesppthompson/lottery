...
export default function Ticket() {
  ...
  const onConnect = async () => {
    console.log("==onConnect func running===");
    try {
      ...
      for (let i = 0; i < userInfo[2].length; i++) {
        const isReward = await contract.viewRewardsForTicketId(
          curLotteryId - 1,
          userInfo[0][i]
        );
        if (isReward > 0) winTickets.push(userInfo[0][i]);
  ...
  const getRandomNum = () => {
    ...
    for (i = 0; i < 6; i++) {
      ...
      if (j < i) i--;
      else randomnum[i] = current;
    }

    let newDice = [...dice];
    for (i = 0; i < 6; i++) newDice[randomnum[i]] = 1;
    return newDice;
  };
  const setRandomNum = () => {
  ...
  const buyTicketOnWeb3 = async (tickets) => {
    ...
    for (let i = 0; i < tickets.length; i++) {
      ...
      for (let j = 0; j < tickets[i].length; j++)
        if (tickets[i][j] == 1) ticket.push(j + 1);
      wrapper.push(ticket);
  ...
  const finishLottery = async () => {
    ...
    const curTicketId = await web3Data.contract.getPlayerCount();
    setCurrentTicketId(curTicketId);
  ...
  const claimReward = async () => {
    ...
    await web3Data.contract.claimReward(
      web3Data.address,
      round,
      claimableTicketId
    );
  ...
  const nextRound = async () => {
    if (round < totalRound - 1) {
      ...
      const userInfo = await web3Data.contract.viewUserInfoForLotteryId(
        web3Data.address,
        round + 1
      );
      console.log(userInfo);
      let winTickets = [];
      for (let i = 0; i < userInfo[2].length; i++) {
        const isReward = await web3Data.contract.viewRewardsForTicketId(
          round + 1,
          userInfo[0][i]
        );
        if (isReward > 0) winTickets.push(userInfo[0][i]);
    ...
    <div>
      ...
      >
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex flex-row h-fit items-center">
                ...
                >
                  {web3Data.address === ""
                    ? "Connect Wallet"
                    : ellipseAddress(web3Data.address, 5)}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex lg:flex-row flex-col gap-2 relative">
                ...
                >
                  {number.map((i) => (
                    <div
                      key={i}
                      className={`${
                        !dice[i] ? "div-unselected " : "div-selected "
          ...
          <div
            className={`${
              lotteryFinished ? "hidden" : ""
            } lg:block lg:w-[50%] w-[100%] relative pb-[100px] lg:pb-0`}
          >
            {web3Data.lotteryInfo.endTime && (
              <Counter
              ...
              <div className="relative w-[100px] md:w-[200px] lg:w-[300px] h-[100px] h-[200px] lg:h-[300px] cube">
                ...
                >
                  <span>
                    Prize
                    <br />${" "}
                    {web3Data.lotteryInfo.amountCollected
                      ? (
                          Web3Utils.fromWei(
                            web3Data.lotteryInfo.amountCollected,
                            "ether"
              ...
              <div className="w-[100px] md:w-[200px] lg:w-[300px] h-[100px] md:h-[200px] lg:h-[300px] scene ml-[calc(50%_-_50px)] md:ml-[calc(50%_-_100px)] lg:ml-[calc(50%_-_150px)] mt-[100%] md:mt-[400px] lg:mt-[400px] flex flex-col items-center">
                <div className="relative w-[100px] md:w-[200px] lg:w-[300px] h-[100px] h-[200px] lg:h-[300px] cube z-[1]">
                  <div className="side back box-neon flex items-center justify-center">
                    <span>
                      {web3Data.lotteryInfo.firstTicketId
                        ? currentTicketId
                        : "0"}
                      <br />
                      players
                ...
                <div className="grid grid-cols-6 gap-2 md:gap-5 w-[300%] md:w-[260%] scale-anim z-[-1] opacity-0">
                  <div className="win-num flex justify-center text-[20px] md:text-[40px] lg:text-[60px]">
                    {web3Data.lotteryInfo.finalNumber[0][0] === "0"
                      ? "?"
                      : web3Data.lotteryInfo.finalNumber[0][0]}
                  </div>
                  <div
                    className="win-num flex justify-center text-[20px] md:text-[40px] lg:text-[60px] "
                    style={{ animationDelay: "500ms" }}
                  >
                    {web3Data.lotteryInfo.finalNumber[0][1] === "0"
                      ? "?"
                      : web3Data.lotteryInfo.finalNumber[0][1]}
                  </div>
                  <div
                    className="win-num flex justify-center text-[20px] md:text-[40px] lg:text-[60px]"
                    style={{ animationDelay: "1000ms" }}
                  >
                    {web3Data.lotteryInfo.finalNumber[0][2] === "0"
                      ? "?"
                      : web3Data.lotteryInfo.finalNumber[0][2]}
                  </div>
                  <div
                    className="win-num flex justify-center text-[20px] md:text-[40px] lg:text-[60px]"
                    style={{ animationDelay: "1500ms" }}
                  >
                    {web3Data.lotteryInfo.finalNumber[0][3] === "0"
                      ? "?"
                      : web3Data.lotteryInfo.finalNumber[0][3]}
                  </div>
                  <div
                    className="win-num flex justify-center text-[20px] md:text-[40px] lg:text-[60px]"
                    style={{ animationDelay: "2000ms" }}
                  >
                    {web3Data.lotteryInfo.finalNumber[0][4] === "0"
                      ? "?"
                      : web3Data.lotteryInfo.finalNumber[0][4]}
                  </div>
                  <div
                    className="win-num flex justify-center text-[20px] md:text-[40px] lg:text-[60px]"
                    style={{ animationDelay: "2500ms" }}
                  >
                    {web3Data.lotteryInfo.finalNumber[0][5] === "0"
                      ? "?"
                      : web3Data.lotteryInfo.finalNumber[0][5]}
                  </div>
                </div>
              </div>
            </div>
      ...
      <div
        className={`${
          darkMode || lotteryFinished ? "bg-gray-800" : "bg-transparent"
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col lg:flex-row px-2 md:px-8">
            ...
            {currentWinningNumber.length && (
              <div className="w-full lg:ml-8 px-2 lg:px-8 py-4 lg:py-0">
          ...
          {claimableTicketId.length ? (
            <div className="flex flex-col lg:flex-row px-2 md:px-8 mt-8">
              ...
              <div className="w-[80%] lg:w-[50%] self-center mt-4 lg:mt-0">
                {!isClaimed ? (
                  <div
                    className="col-span-3 flex justify-center text-white border-solid border-2 border-white rounded-xl text-xl lg:text-2xl p-1 lg:font-bold lg:font-bold claim-button"
                    onClick={claimReward}
                  >
                    Claim
                  </div>
